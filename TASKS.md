...
- [ ] 子功能 3.1：设置弹窗基础架构
  ...
  - 实现设置数据的统一管理Hook
  - 实现设置界面的响应式布局
  - 测试方法：点击设置按钮弹出设置窗口，界面布局正常
  - 备注：
    - 使用Shadcn/UI Dialog组件构建设置弹窗
    - 左侧导航菜单，右侧设置内容区域
    - 支持设置项的分组管理和搜索功能
    - 集成现有的毛玻璃效果和主题系统
...
- [ ] 子功能 3.3：书签与分类设置
  ...
  - **与其他模块关联**：
    - 集成分类管理系统的配置界面
    - 连接书签系统的批量操作功能
  - 测试方法：通过设置修改书签显示，验证书签网格更新

- [ ] 子功能 3.4：背景图片设置
  - 实现背景图片来源选择（本地/Unsplash）
  - 实现Unsplash关键词和刷新频率设置
  - 实现本地背景图片上传管理
  - 实现背景图片效果设置（模糊度、暗化等）
  - **与其他模块关联**：
    - 集成背景图片系统的配置选项
    - 连接Unsplash API的参数设置
  - 测试方法：修改背景设置，验证背景图片实时更新
...
- [ ] 子功能 5.2：Unsplash API集成
  ...
  - 实现图片缓存系统
  - 实现API限额监控与回退机制
  - **设置集成**：添加Unsplash配置到设置弹窗

- [ ] 子功能 5.3：背景图片归属信息
  - 实现图片信息展示组件
  - 实现归属信息的获取与显示
  - **设置集成**：添加归属信息显示选项到设置弹窗
